-- ============================================================================
-- Multi-Tenant SaaS Setup with Subscription Management
-- ============================================================================
...
ALTER TABLE public.rider_stock ADD COLUMN organization_id UUID REFERENCES public.organizations(id) ON DELETE CASCADE;
ALTER TABLE public.distributions ADD COLUMN organization_id UUID REFERENCES public.organizations(id) ON DELETE CASCADE;
ALTER TABLE public.returns ADD COLUMN organization_id UUID REFERENCES public.organizations(id) ON DELETE CASCADE;
ALTER TABLE public.transactions ADD COLUMN organization_id UUID REFERENCES public.organizations(id) ON DELETE CASCADE;
ALTER TABLE public.production_history ADD COLUMN organization_id UUID REFERENCES public.organizations(id) ON DELETE CASCADE;

-- 3. Create super_admin role (extends app_role enum)
ALTER TYPE public.app_role ADD VALUE IF NOT EXISTS 'super_admin';

-- 4. Create subscription_plans table (untuk pricing)
...
-- 5. Create subscription_history table (untuk tracking pembayaran)
...
-- 7. Function to check if organization subscription is active
...
-- 8. Function to check organization limits
...
-- 9. Update handle_new_user function to support multi-tenant
...
-- 12. RLS Policies for subscription_plans
...
-- 13. Update existing RLS policies to support multi-tenant
...
-- 15. Create trigger to update organization updated_at
